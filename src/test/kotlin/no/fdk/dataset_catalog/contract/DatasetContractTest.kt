...
class DatasetContractTest : ApiTestContext() {
    ...
    internal inner class CreateDataset {
        ...
        fun `Illegal create`() {
            val notLoggedIn = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets",
        ...
        fun `Invalid create`() {
            ...
            val nonExistingCatalog = apiAuthorizedRequest(
                "/catalogs/$CATALOG_ID_2/datasets",
        ...
        fun `Able to get after create`() {
            val rspCreate = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets",
                mapper.writeValueAsString(DATASET_1),
                JwtToken(Access.ORG_WRITE).toString(),
                "POST"
            ...
            val rspGet = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets/$DATASET_ID_1",
                null,
                JwtToken(Access.ORG_WRITE).toString(),
                "GET"
            )
        ...
        fun `All fields are persisted`() {
            ...
            val rspGet = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets/$DATASET_ID_2",
                null,
                JwtToken(Access.ORG_WRITE).toString(),
                "GET"
            )
    ...
    internal inner class GetDataset {
        ...
        fun `Unable to get when not logged in as a user with org access`() {
            resetDB()
            val notLoggedIn =
                apiAuthorizedRequest("/catalogs/$DB_CATALOG_ID_1/datasets/${DATASET_ID_1}", null, null, "GET")
            val wrongOrg = apiAuthorizedRequest(
                "/catalogs/1/datasets/${DATASET_ID_1}",
                null,
                JwtToken(Access.ORG_READ).toString(),
                "GET"
            )
        ...
        fun `Both read and write can read`() {
            resetDB()
            val rspRead = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets/${DB_DATASET_ID_1}",
                null,
                JwtToken(Access.ORG_READ).toString(),
                "GET"
            )
            val rspWrite = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets/${DB_DATASET_ID_1}",
                null,
                JwtToken(Access.ORG_WRITE).toString(),
                "GET"
        ...
        fun `Get All datasets returns all datasets in catalog`() {
            resetDB()
            val rspRead = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_2/datasets",
                null,
            ...
            assertEquals(
                ...
                bodyRead._embedded?.get("datasets")?.map { it.id })
        }
    }
    ...
    internal inner class UpdateDataset {
        ...
        fun `Illegal update`() {
            ...
            val notLoggedIn =
                apiAuthorizedRequest("/catalogs/$DB_CATALOG_ID_1/datasets/${DB_DATASET_ID_1}", body, null, "PATCH")
            val readAccess = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets/${DB_DATASET_ID_1}",
                body,
                JwtToken(Access.ORG_READ).toString(),
            ...
            val wrongOrg = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_2/datasets/${DB_DATASET_ID_1}",
                body,
                JwtToken(Access.ORG_WRITE).toString(),
                "PATCH"
            )
        ...
        fun `Invalid update`() {
            ...
            val doesNotExist = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets/${DB_DATASET_ID_4}",
    ...
    internal inner class Themes {
        ...
        fun `losTheme and euDataTheme is saved correctly`() {
            val rspCreate = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets",
                mapper.writeValueAsString(DATASET_3),
                JwtToken(Access.ORG_WRITE).toString(),
                "POST"
            )
            Assumptions.assumeTrue(HttpStatus.CREATED.value() == rspCreate["status"])
            ...
            val rspGet = apiAuthorizedRequest(
                "/catalogs/$DB_CATALOG_ID_1/datasets/$DATASET_ID_3",
                null,
                JwtToken(Access.ORG_WRITE).toString(),
                "GET"
            )
